...
export const RUBRIC_VERSIONS = ['V1', 'V2', 'V3'];
...
// ═══════════════════════════════════════════════════════════════════════════════
// V3 — NUQUEST + Cochrane RoB 2 Domain Mapping Support
//
// Extends V2 with nutrition-specific items and the trial-registration
// check required to complete the RoB 2 Domain 5 mapping without the
// "registration unknown" caveat. Phase 1 introduces Q12 only; Q13–Q18
// (NUQUEST nutrition items, funding/COI, selective-reporting proxy)
// land in Phase 5 of the approved plan.
//
// Reference: Kelly SE et al. Am J Clin Nutr 2022;115(6):1457 (NUQUEST).
// ═══════════════════════════════════════════════════════════════════════════════

export const RUBRIC_QUESTIONS_V3 = [
  ...RUBRIC_QUESTIONS_V2,
  {
    id: 'q12', number: 12, label: 'Trial Registration', property: 'Q12 Trial Registration',
    description: 'Was the trial prospectively registered in a public registry (ClinicalTrials.gov, ISRCTN, CTRI, ANZCTR, EU CTR, UMIN, PACTR) with a registration number verifiable against the reported protocol?',
    tier: 4, tierLabel: 'Completes RoB 2 D5 Mapping',
    options: [
      { score: 2, tier: 'High', label: '2 — High',
        notionValue: '2 — High [V3]: Registered prospectively (before enrollment) in a public registry; registration number cited; primary outcomes in registry match paper',
        criteria: ['Registered prospectively in a public registry (ClinicalTrials.gov, ISRCTN, CTRI, etc.)', 'Registration number cited in paper', 'Primary outcomes in registry match those reported'] },
      { score: 1, tier: 'Moderate', label: '1 — Moderate',
        notionValue: '1 — Moderate [V3]: Retrospectively registered, OR registration number provided but registry entry cannot be matched to paper',
        criteria: ['Retrospectively registered (after enrollment began)', 'OR registration number provided but protocol mismatch', 'OR pre-published protocol exists even without registry entry'] },
      { score: 0, tier: 'Low', label: '0 — Low',
        notionValue: '0 — Low [V3]: No trial registration reported; no published protocol',
        criteria: ['No trial registration reported', 'No published protocol available'] },
    ],
  },
];
 ...
 * @param {string} version - 'V1', 'V2', or 'V3' (defaults to DEFAULT_RUBRIC_VERSION)
 */
export function getRubricByVersion(version) {
  if (version === 'V1') return RUBRIC_QUESTIONS_V1;
  if (version === 'V3') return RUBRIC_QUESTIONS_V3;
 ...
 * V2 notionValues contain "[V2]" marker; V3 contain "[V3]".
 * @param {string} notionValue - A notionValue string from a score
 * @returns {string} 'V1', 'V2', or 'V3'
 */
export function detectVersionFromNotionValue(notionValue) {
  if (notionValue && notionValue.includes('[V3]')) return 'V3';
