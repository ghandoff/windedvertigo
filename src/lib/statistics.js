...
// ─── RoB 2 Domain Mapping (article-level aggregation) ──────────────────

import { mapRubricToRoB2 } from './rob2-mapping';

/**
 * Aggregate per-rater scores into a single article-level RoB 2
 * judgment using the mean rubric score per question (rounded to the
 * nearest integer to preserve the 0/1/2 categorical form).
 *
 * Mean-and-round is deliberate: it matches how RoB 2 is applied
 * in practice — one judgment per study, not per rater — while
 * staying faithful to the underlying distribution. Disagreement
 * between raters still surfaces via the existing consensus-spread
 * statistics; this is a complementary translation, not a reliability
 * metric.
 *
 * @param {Array} reviewerScores - array of per-rater score objects
 *   each containing q1..q11 (and optional q12 for V3)
 * @returns {Object|null} RoB 2 output, or null if no scores
 */
export function aggregateRoB2ForArticle(reviewerScores) {
  if (!reviewerScores || reviewerScores.length === 0) return null;

  const aggregated = {};
  const qIds = ['q1', 'q2', 'q3', 'q4', 'q5', 'q6', 'q7', 'q8', 'q9', 'q10', 'q11', 'q12'];
  for (const q of qIds) {
    const values = reviewerScores.map(s => s[q]).filter(v => v === 0 || v === 1 || v === 2);
    if (values.length === 0) continue;
    const mean = values.reduce((a, b) => a + b, 0) / values.length;
    aggregated[q] = Math.round(mean);
  }

  // Translate aggregated q12 (0/1/2) into the boolean/string shape
  // mapRubricToRoB2 understands for D5 registration check.
  if (aggregated.q12 === 2) aggregated.q12 = true;
  else if (aggregated.q12 === 0) aggregated.q12 = false;
  // q12 === 1 (retrospectively registered) remains numeric;
  // mapRubricToRoB2 treats it as registrationUnknown, which produces
  // "Some concerns" — the conservative default for partial evidence.

  const result = mapRubricToRoB2(aggregated);
  return {
    ...result,
    aggregatedScores: aggregated,
    raterCount: reviewerScores.length,
  };
}
