...
| id | feature | status | surface | fixes | source |
|----|---------|--------|---------|-------|--------|
| BIZ-B1 | CV de-dup — block identical copy-pasted experience entries | ⬜ | 🤖⚙️ | maria's #1 credibility issue | loopio ai librarian, ombud |
...
| id | feature | status | surface | fixes | source |
|----|---------|--------|---------|-------|--------|
| BIZ-D1 | `biz_qc_review`: red-team draft vs the funder's own evaluation rubric | ⬜ | 🤖 | **no QC gate** | deeprfp color, autogenai gamma, granter |
| BIZ-D2 | **cross-source conflict detection** (deal-page ↔ docs ↔ within-draft) | ⬜ novel | 🤖📊 | "60 countries vs asia-pacific / 8wk vs 7mo" | inventive ai (market whitespace) |
| BIZ-D3 | requirements-coverage check (every `rfp_requirements` row answered) | 🟡 `rfp_coverage` exists | 🤖📊 | incomplete bids | ombud, responsive |
