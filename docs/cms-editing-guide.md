...
The winded.vertigo website and harbor apps are powered by a Notion database called **Site Content CMS**. Editing a row in Notion changes what appears on the live site after a sync. This guide explains every field, how they interact, and what to expect when you make changes.
...
The JSON files live at `apps/site/data/site-content-{page}.json`. One file per page value (e.g., `what`, `we`, `do`, `home`, `harbor`).
...
| **Page** | Select | Which page this content belongs to: `home`, `what`, `we`, `do`, or `harbor`. This is how the sync groups content into separate JSON files. |
...
| **Brand Color** | Select | A named brand color (`sienna`, `redwood`, `cadet`). Currently used on harbor game cards to set the primary color. |
...
|------|---------|-------------------|
| `hero` | what, do, harbor | Large centered heading text — the page's main statement |
...
| `game-card` | harbor | An interactive game card with Name, Content (description), Brand/Accent Colors, Icon, and Link |
| `credential` | harbor | A credential or qualification badge with Name and Icon |
| `principle` | harbor | A design principle with Name (title) and Content (explanation) |
...
### Add a new game to the harbor
1. Create a new row with **Page** = `harbor`, **Content Type** = `game-card`, **Section** = `games`
