...
- **Reactive `@`-mention path was 401ing on Workers — FIXED (PR pending deploy).** `lib/agent/index.ts` built its Anthropic client at **module scope** (`new Anthropic()`), but Worker env isn't available at isolate-init, so it had no key and fell back to the Vercel AI Gateway → `401 AI_GATEWAY_API_KEY` on every `@`-mention/DM (migration debt — the crons were fine, they use the lazy `getAnthropic()`). Fix: `index.ts` now reuses the exported lazy `getAnthropic()` per-request. The ambient spine was never affected (it uses `callClaude`). Deploy + `@`-mention an agent to confirm the reply is real, not a 401.
