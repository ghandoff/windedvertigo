...
/**
 * Shared site footer with copyright and social links.
 * Accepts optional CMS sections for dynamic copyright/social URLs.
 */

// SVG social icons (replaces Font Awesome CDN dependency)
...
const DEFAULT_SOCIAL = [
  { label: "instagram", href: "https://www.instagram.com/winded.vertigo/" },
...
export function SiteFooter({ sections }: { sections?: SiteSection[] }) {
  ...
  const copyright =
    ...
    "© copyright winded.vertigo, 2024–2026. all rights reserved.";

  // Build social links with CMS URL overrides
  const socialOverrides = new Map<string, string>();
  sections
    ?.filter((s) => s.type === "footer" && s.section === "social" && s.link)
    .forEach((s) => {
      const label = s.name.replace("social: ", "").toLowerCase();
      socialOverrides.set(label, s.link);
    });

  const social = DEFAULT_SOCIAL.map((s) => ({
    ...s,
    href: socialOverrides.get(s.label) ?? s.href,
  }));
