import { Meeting } from '@/lib/data';

interface MeetingCardProps {
  meeting: Meeting;
  index: number;
}

export function MeetingCard({ meeting, index }: MeetingCardProps) {
  return (
    <div
      className="card-animate p-4 bg-ops-card border border-ops-border rounded-lg"
      style={{ animationDelay: `${index * 50}ms` }}
    >
      <div className="flex items-start justify-between mb-2">
        <h4 className="font-semibold text-ops-text text-sm">{meeting.title}</h4>
        <span className="text-xs text-ops-text-muted">{meeting.day}</span>
      </div>
      <p className="text-sm text-ops-text mb-3">
        <span className="font-mono">{meeting.time}</span>
        <span className="text-ops-text-muted ml-2">{meeting.timezone}</span>
      </p>
      {meeting.attendees && meeting.attendees.length > 0 && (
        <div className="text-xs text-ops-text-muted">
          {meeting.attendees.join(', ')}
        </div>
      )}
    </div>
  );
}
