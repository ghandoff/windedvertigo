...
/** BFS down from rootId to collect all descendants + their spouses */
function getDescendants(rootId: string, treeNodes: TreeNode[]): TreeNode[] {
  ...
  while (queue.length > 0) {
    ...
    // include spouses (so couple unions render correctly)
    for (const spouseId of node.spouseIds) {
      if (!included.has(spouseId)) included.add(spouseId);
    }
  }

  // filter treeNodes to only included ids, and trim parent/spouse/child refs to included set
  return treeNodes
...
function DescendancyChartInner({
  nodes: treeNodes,
  rootId,
  colorMode = "sex",
  events = [],
  ...
  const filteredNodes = useMemo(
    () => getDescendants(rootId, treeNodes),
    [rootId, treeNodes],
  );
  ...
  // keyboard: Enter navigates to person page
  useEffect(() => {
...
export function DescendancyChart({
  nodes,
  rootId,
  colorMode,
  events,
  ...
  return (
    <div ref={containerRef} className="w-full h-full">
      {ready ? (
        <ReactFlowProvider>
          <DescendancyChartInner nodes={nodes} rootId={rootId} colorMode={colorMode} events={events} />
        </ReactFlowProvider>
      ) : (
