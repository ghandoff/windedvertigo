-- ancestry schema v1
-- run against neon postgres to initialize

CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- trees (top-level container)
CREATE TABLE trees (
...
CREATE TABLE persons (
    ...
    privacy_level   TEXT DEFAULT 'private'
                    CHECK (privacy_level IN ('public', 'authenticated', 'family', 'private')),
    thumbnail_url   TEXT,
    notes           TEXT,
    created_at      TIMESTAMPTZ DEFAULT now(),
    updated_at      TIMESTAMPTZ DEFAULT now()
...
-- names (multiple per person: birth, married, alias, etc.)
CREATE TABLE person_names (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    person_id   UUID NOT NULL REFERENCES persons(id) ON DELETE CASCADE,
    name_type   TEXT DEFAULT 'birth'
                CHECK (name_type IN ('birth', 'married', 'adopted', 'alias', 'nickname', 'other')),
    given_names TEXT,
    surname     TEXT,
    prefix      TEXT,
    suffix      TEXT,
...
CREATE INDEX idx_person_names_search ON person_names(LOWER(surname), LOWER(given_names));

-- relationships (direct person-to-person, not GEDCOM FAM model)
...
-- places (hierarchical: country > state > county > city)
CREATE TABLE places (
    id          UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tree_id     UUID NOT NULL REFERENCES trees(id) ON DELETE CASCADE,
...
CREATE INDEX idx_events_sort ON events(sort_date);
...
-- citations (link events to sources)
