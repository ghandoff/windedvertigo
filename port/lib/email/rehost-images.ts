 ...
 * An injected <img src="http://169.254.169.254/..."> in org content would
 * otherwise cause the server to hit AWS/GCP instance metadata endpoints.
 */
...
async function rehostOne(src: string): Promise<string> {
  if (isPrivateUrl(src)) throw new Error(`Blocked private/internal URL: ${src}`);
