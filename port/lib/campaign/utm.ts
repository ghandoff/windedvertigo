 ...
 *
 * When `orgId` is provided, it's appended to utm_content for attribution:
 * the email→click→site-visit chain can be traced back to the org.
 * Format: "variant-a_org-{id}" or "org-{id}" (when no A/B variant).
 */
export function buildEmailUtmParams(
  campaignName: string,
  variant?: "a" | "b",
  orgId?: string,
): UtmParams {
  ...
  if (variant) contentParts.push(`variant-${variant}`);
  if (orgId) contentParts.push(`org-${orgId}`);
