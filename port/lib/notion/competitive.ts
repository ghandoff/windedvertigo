...
import { notion, PORT_DB, COMPETITIVE_PROPS } from "./client";
...
export async function queryCompetitors(
  ...
  const result = await queryDatabase(notion, {
    database_id: PORT_DB.competitive,
...
export async function createCompetitor(
  ...
  const page = (await notion.pages.create({
    parent: { data_source_id: PORT_DB.competitive },
    properties,
...
/** Fetch every competitor for Supabase sync. */
export async function getAllCompetitors(): Promise<Competitor[]> {
  const result = await queryDatabase(notion, {
    ...
    page_size: 200,
    fetchAll: true,
    label: "getAllCompetitors",
  });
  return result.pages.map(mapPageToCompetitor);
}
