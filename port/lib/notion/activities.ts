...
import { notion, PORT_DB, ACTIVITY_PROPS } from "./client";
...
export async function queryActivities(
  ...
  const result = await queryDatabase(notion, {
    database_id: PORT_DB.activities,
...
export async function createActivity(
  ...
  const page = (await notion.pages.create({
    parent: { data_source_id: PORT_DB.activities },
    properties,
...
export async function getAllActivities(): Promise<Activity[]> {
  const result = await queryDatabase(notion, {
    ...
    page_size: 500,
    label: "getAllActivities",
  });
  return result.pages.map(mapPageToActivity);
}
