...
import { notion, PORT_DB, EVENT_PROPS } from "./client";
...
export async function createEvent(
  ...
  const page = (await notion.pages.create({
    parent: { data_source_id: PORT_DB.events },
    properties,
...
/** Fetch every event for Supabase sync. */
export async function getAllEvents(): Promise<CrmEvent[]> {
  const result = await queryDatabase(notion, {
    database_id: PORT_DB.events,
  ...
  return result.pages.map(mapPageToEvent);
}
