...
import { notion, PORT_DB, SOCIAL_PROPS } from "./client";
...
export async function querySocialDrafts(
  ...
  const result = await queryDatabase(notion, {
    database_id: PORT_DB.socialQueue,
...
export async function createSocialDraft(
  ...
  const page = (await notion.pages.create({
    parent: { data_source_id: PORT_DB.socialQueue },
    properties,
...
export async function getAllSocialDrafts(): Promise<SocialDraft[]> {
  const result = await queryDatabase(notion, {
    database_id: PORT_DB.socialQueue,
    sorts: [{ timestamp: "last_edited_time", direction: "descending" }],
    page_size: 200,
    label: "getAllSocialDrafts",
  });
  return result.pages.map(mapPageToSocialDraft);
}
