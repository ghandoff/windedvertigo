...
import { notion, PORT_DB, CONTACT_PROPS } from "./client";
...
function mapPageToContact(page: PageObjectResponse): Contact {
  ...
  return {
    ...
    profilePhotoUrl: getUrl(props[P.profilePhotoUrl]),
...
export async function getAllContacts(): Promise<Contact[]> {
  ...
  return result.pages.map(mapPageToContact);
}
...
export async function createContact(
  ...
  if (fields.profilePhotoUrl) properties[P.profilePhotoUrl] = buildUrl(fields.profilePhotoUrl);
  ...
  const page = (await notion.pages.create({
    parent: { data_source_id: PORT_DB.contacts },
    properties,
...
export async function updateContact(
  ...
  if (fields.profilePhotoUrl !== undefined) properties[P.profilePhotoUrl] = buildUrl(fields.profilePhotoUrl);
