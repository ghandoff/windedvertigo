...
import { notion, PORT_DB, BD_ASSET_PROPS } from "./client";
...
export async function createBdAsset(
  ...
  const page = (await notion.pages.create({
    parent: { data_source_id: PORT_DB.bdAssets },
    properties,
...
export async function incrementBdAssetUsage(assetId: string): Promise<void> {
  try {
    const page = (await notion.pages.retrieve({ page_id: assetId })) as PageObjectResponse;
    const current = getNumber(page.properties[P.timesUsed]) ?? 0;
    await notion.pages.update({
      page_id: assetId,
      properties: {
        [P.timesUsed]: buildNumber(current + 1),
      },
    });
  } catch {
    // Never throws — usage tracking is non-fatal
  }
}

/** Fetch every BD asset for Supabase sync. */
export async function getAllBdAssets(): Promise<BdAsset[]> {
  const result = await queryDatabase(notion, {
    database_id: PORT_DB.bdAssets,
    sorts: [{ timestamp: "last_edited_time", direction: "descending" }],
    page_size: 200,
    fetchAll: true,
    label: "getAllBdAssets",
  });
  return result.pages.map(mapPageToBdAsset);
}
