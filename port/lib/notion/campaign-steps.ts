...
import { notion, PORT_DB, CAMPAIGN_STEP_PROPS } from "./client";
import type {
  CampaignStep,
  CampaignStepFilters,
  PaginationParams,
...
function mapPageToStep(page: PageObjectResponse): CampaignStep {
  ...
  return {
    ...
    sentCount: getNumber(props[P.sentCount]),
    skippedCount: getNumber(props[P.skippedCount]),
    failedCount: getNumber(props[P.failedCount]),
...
export async function queryCampaignSteps(
  ...
  const result = await queryDatabase(notion, {
    database_id: PORT_DB.campaignSteps,
...
export async function createCampaignStep(
  ...
  const page = (await notion.pages.create({
    parent: { data_source_id: PORT_DB.campaignSteps },
    properties,
...
export async function updateCampaignStep(
  ...
  if (fields.sentCount !== undefined) properties[P.sentCount] = buildNumber(fields.sentCount);
  if (fields.skippedCount !== undefined) properties[P.skippedCount] = buildNumber(fields.skippedCount);
  if (fields.failedCount !== undefined) properties[P.failedCount] = buildNumber(fields.failedCount);
...
/** Get all campaign steps — used by the Supabase sync cron. */
export async function getAllCampaignSteps(): Promise<CampaignStep[]> {
  const result = await queryDatabase(notion, {
    database_id: PORT_DB.campaignSteps,
    sorts: [{ property: P.stepNumber, direction: "ascending" }],
    page_size: 500,
    fetchAll: true,
    label: "getAllCampaignSteps",
  });
  return result.pages.map(mapPageToStep);
}
