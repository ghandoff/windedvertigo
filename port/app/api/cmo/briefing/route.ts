  ...
  try {
    ...
    if (decisions.length === 0) {
      ...
      for (const d of decisions) {
        ...
        if (d.tags?.length) {
          lines.push(`_tags: ${d.tags.join(", ")}_`);
        }
        lines.push("");
      }
    }

    return json({ briefing: lines.join("\n"), decisions_count: decisions.length, memory_count: memory.length });
  } catch (err) {
    console.error("[api/cmo/briefing] GET failed:", err);
