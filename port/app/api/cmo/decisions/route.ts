...
import { insertCmoDecision, getCmoDecisions } from "@/lib/supabase/cmo";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const decisions = await getCmoDecisions({ days, who, tag });
    return json(decisions);
  } catch (err) {
    console.error("[api/cmo/decisions] GET failed:", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await insertCmoDecision({
      who: body.who,
    ...
    console.error("[api/cmo/decisions] POST failed:", err);
