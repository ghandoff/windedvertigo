...
import { insertCarlDecision, getCarlDecisions } from "@/lib/supabase/carl";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const decisions = await getCarlDecisions({ days, who, tag });
    return json(decisions);
  } catch (err) {
    console.error("[api/carl/decisions] GET failed:", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await insertCarlDecision({
      who: body.who,
    ...
    console.error("[api/carl/decisions] POST failed:", err);
