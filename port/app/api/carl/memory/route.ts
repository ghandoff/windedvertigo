...
import { upsertCarlMemory, getCarlMemory } from "@/lib/supabase/carl";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const memory = await getCarlMemory();
    return json(memory);
  } catch (err) {
    console.error("[api/carl/memory] GET failed:", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await upsertCarlMemory(body.key, body.value, body.updated_by);
    return json(result);
  } catch (err) {
    console.error("[api/carl/memory] POST failed:", err);
