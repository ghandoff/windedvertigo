...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Derive first name from the primary recipient's contact name when provided.
    // Fallback chain: contactName → org name first word → empty string.
    ...
    // Guard: if Resend returned an error, mark draft as failed and bail out.
    // sendOutreachEmail returns { data, error } — the error is in the return
    // value, not thrown, so the outer catch block won't catch it.
    if (result.error) {
      if (draft) {
        await updateEmailDraft(draft.id, {
          ...
          console.warn("[email/send] draft fail-update:", err instanceof Error ? err.message : err),
        );
      }
      return error(`Send failed: ${result.error.message}`, 502);
    }
    ...
    if (org?.contactIds?.length) {
      try {
        await createActivity({
          ...
          notes: `sent via port to ${to.join(", ")}`,
          loggedBy: body.senderName || "port",
        });
      } catch {
