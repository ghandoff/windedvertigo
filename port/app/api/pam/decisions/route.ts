...
import { insertPamDecision, getPamDecisions } from "@/lib/supabase/pam";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const decisions = await getPamDecisions({ days, who, tag });
    return json(decisions);
  } catch (err) {
    console.error("[api/pam/decisions] GET failed:", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await insertPamDecision({
      who: body.who,
    ...
    console.error("[api/pam/decisions] POST failed:", err);
