...
import { upsertPamMemory, getPamMemory } from "@/lib/supabase/pam";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const memory = await getPamMemory();
    return json(memory);
  } catch (err) {
    console.error("[api/pam/memory] GET failed:", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await upsertPamMemory(body.key, body.value, body.updated_by);
    return json(result);
  } catch (err) {
    console.error("[api/pam/memory] POST failed:", err);
