...
import { getBizMemory, upsertBizMemory } from "@/lib/biz-data";
...
export async function GET(req: NextRequest) {
  ...
  try {
    const memory = await getBizMemory();
    return json(memory);
  } catch (err) {
    console.error("[api/biz/memory] GET failed:", err);
...
export async function POST(req: NextRequest) {
  ...
  try {
    const result = await upsertBizMemory(body.key, body.value, body.updated_by);
    return json(result);
  } catch (err) {
    console.error("[api/biz/memory] POST failed:", err);
