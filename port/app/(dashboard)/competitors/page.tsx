...
async function CompetitorsList({ searchParams }: Props) {
  ...
  if (competitors.length === 0) {
    return (
      <EmptyState
        ...
        description="try adjusting your filters or add a competitor to start tracking the landscape."
      />
    );
  }
  ...
  // Ungrouped (no threat level set)
  const ungrouped = competitors.filter((c) => !c.threatLevel);

  return (
    <>
      {/* Pass full list to actions so AI generate knows what already exists */}
