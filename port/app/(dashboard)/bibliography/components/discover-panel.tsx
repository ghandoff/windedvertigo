...
// Smart find: paste a DOI (→ exact match), or a title / author / full citation
// (→ federated search). One field; the server action routes it. One-click add
// fires background open-access PDF retrieval.
export function DiscoverPanel({ allAssets }: { allAssets: string[] }) {
    ...
    <div className="space-y-3">
      <form onSubmit={(e) => { e.preventDefault(); search(); }} className="flex gap-1.5">
        <Input
          ...
          placeholder="paste a DOI, article title, author, or full citation…"
        />
      ...
      {exact && hits.length > 0 && (
        <p className="text-[10px] text-muted-foreground">exact DOI match · crossref</p>
      )}
