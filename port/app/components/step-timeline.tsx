...
import { Mail, Globe, Hash, Cloud, Plus, Send, CheckCircle2, AlertCircle, RotateCcw } from "lucide-react";
...
import { EmailPreviewModal } from "@/app/components/email-preview-modal";
...
export function StepTimeline({ campaignId, steps, audienceCount }: StepTimelineProps) {
  ...
  const [resettingCounts, setResettingCounts] = useState<string | null>(null);
  ...
  async function handleResetStep(stepId: string) {
    setResettingStep(stepId);
    try {
      await fetch(`/api/campaign-steps/${stepId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ status: "draft", sendDate: null }),
      });
      startTransition(() => router.refresh());
    } finally {
  ...
  async function handleResetCounts(stepId: string) {
    setResettingCounts(stepId);
    try {
      await fetch(`/api/campaign-steps/${stepId}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ sentCount: 0, skippedCount: 0, failedCount: 0 }),
      });
      setSendResult(null);
      startTransition(() => router.refresh());
    } finally {
      setResettingCounts(null);
    }
  }
  ...
  async function handleSendStep(stepId: string) {
    ...
    try {
      ...
      if (res.ok) {
        const summary = `sent ${data.sent}, skipped ${data.skipped}, failed ${data.failed}`;
        const detail = data.failed > 0 && data.firstError ? ` — ${data.firstError}` : "";
        setSendResult({ stepId, message: summary + detail, ok: data.failed === 0 });
        startTransition(() => router.refresh());
      } else {
              ...
              <div className="flex-1 min-w-0 space-y-1">
                ...
                {step.body && (
                  <p className="text-xs text-muted-foreground line-clamp-2">
                    {step.body.trimStart().startsWith("<") ? "[html email]" : step.body}
                  </p>
                )}
                ...
                {/* send result — ephemeral flash right after sending */}
                ...
                {/* persistent send stats from Notion — shown on sent steps when no ephemeral result */}
                {step.status === "sent" && sendResult?.stepId !== step.id && step.sentCount != null && (
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
                      <CheckCircle2 className="h-3.5 w-3.5 text-green-600" />
                      sent {step.sentCount}, skipped {step.skippedCount ?? 0}, failed {step.failedCount ?? 0}
                    </div>
                    <button
                      onClick={() => handleResetCounts(step.id)}
                      disabled={resettingCounts === step.id}
                      className="text-[10px] text-muted-foreground hover:text-foreground transition-colors disabled:opacity-50"
                      title="Reset sent/skipped/failed counts to 0"
                    >
                      {resettingCounts === step.id ? "resetting…" : "reset counts"}
                    </button>
                  </div>
                )}
                {/* reset to draft — lets you re-queue a sent/failed step */}
                {step.status === "sent" && (
                  <button
                    onClick={() => handleResetStep(step.id)}
                    disabled={resettingStep === step.id}
                    className="flex items-center gap-1 text-[10px] text-muted-foreground hover:text-foreground mt-1 transition-colors disabled:opacity-50"
                    title="Reset to draft to re-send"
                  >
                    <RotateCcw className="h-3 w-3" />
                    {resettingStep === step.id ? "resetting..." : "re-queue"}
                  </button>
                )}
                {/* preview as — show rendered email with substituted variables */}
                {step.channel === "email" && step.body && (
                  <EmailPreviewModal subject={step.subject ?? ""} body={step.body} />
                )}
              </div>
            </div>
          </div>
