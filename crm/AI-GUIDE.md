# w.v CRM — AI Integration Guide

## Overview

The w.v CRM at `windedvertigo.com/crm` is a Next.js 16 app backed by Notion databases, with AI features powered by the Claude API (Anthropic). AI is embedded throughout the CRM — not isolated to a single page.
...
These use NO AI API calls — computed from existing CRM data:
...
| Pattern | Industry Standard | w.v CRM |
|---------|-------------------|---------|
...
### Long-term
- [ ] Conversational AI sidebar (ask anything about CRM data)
...
```bash
# Run CRM locally
npm run dev:crm         # starts on port 3005
