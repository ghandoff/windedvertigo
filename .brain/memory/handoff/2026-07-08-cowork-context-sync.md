...
- **handoff-file hygiene gap confirmed and fixed** — checked `git log --all` for the dated session files (`2026-06-18` through `2026-07-07`): none were ever actually committed, despite the 7 jul session's note claiming they'd been committed. `_live-state.md` itself was also sitting with an uncommitted local edit (last real commit reflected the 30 jun snapshot, not 7 jul's). Committed all of it — the backlog of dated session files plus `_live-state.md` plus this file — as part of this run.
