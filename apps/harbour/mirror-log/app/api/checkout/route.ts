 ...
 *
 * POST — create a Stripe Checkout Session for a mirror-log pack purchase.
...
export async function POST(req: NextRequest) {
  ...
  try {
    const url = await createHarbourCheckout({
      app: "mirror-log",
      appUrl: process.env.NEXT_PUBLIC_APP_URL || "https://windedvertigo.com/harbour/mirror-log",
      userId: session.user.id ?? "",
      email: session.user.email,
      userName: session.user.name,
      packCacheId,
      catalogueId: packCacheId,
      packTitle: "mirror.log pack",
      priceCents: 0,
      currency: "USD",
    });
