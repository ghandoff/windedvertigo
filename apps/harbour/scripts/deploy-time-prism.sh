#!/usr/bin/env bash
# Deploy the time-prism Vercel project from the monorepo root.
#
# Usage:
#   ./scripts/deploy-time-prism.sh            # production deploy
#   ./scripts/deploy-time-prism.sh --preview  # preview deploy
...
PROJECT_ID="prj_17rEjLAm982rrfPHGE4OyCgK3FVi"
...
echo "→ Swapping .vercel/project.json to target time-prism"
...
printf '{"projectId":"%s","orgId":"%s","projectName":"time-prism"}\n' \
