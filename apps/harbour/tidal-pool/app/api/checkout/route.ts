 ...
 *
 * POST — create a Stripe Checkout Session for a tidal-pool pack purchase.
...
export async function POST(req: NextRequest) {
  ...
  try {
    const url = await createHarbourCheckout({
      app: "tidal-pool",
      appUrl: process.env.NEXT_PUBLIC_APP_URL || "https://windedvertigo.com/harbour/tidal-pool",
      userId: session.user.id ?? "",
      email: session.user.email,
      userName: session.user.name,
      packCacheId,
      catalogueId: packCacheId, // will be resolved from DB in future
      packTitle: "tidal.pool pack",
      priceCents: 0,
      currency: "USD",
    });
