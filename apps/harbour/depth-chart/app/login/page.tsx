...
function LoginInner() {
  ...
  // Auth.js v5 — use the client signIn helper which fetches the CSRF token
  // and handles redirects. The previous v4-style POST form with an empty
  // hidden csrfToken input failed with MissingCSRF on CF Workers.
  const fullCallback = `/harbour/depth-chart${callback_url}`;
    ...
    <main id="main" className="min-h-screen flex items-center justify-center px-6">
      <div className="max-w-md w-full space-y-8">
        <div className="text-center space-y-2">
          <h1 className="text-2xl font-bold text-[var(--color-text-on-dark)]">
            sign in to depth.chart
          </h1>
          <p className="text-sm text-[var(--color-text-on-dark-muted)]">
            save your plans, track history, and export assessments.
          </p>
        </div>
