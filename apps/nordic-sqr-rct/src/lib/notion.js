 ...
 * Used by /api/auth/confirm-email to enforce uniqueness in code, since
 * Notion does not enforce DB-level uniqueness on the Email property.
 */
...
// Check if an intake record already exists for a DOI (any submitter)
...
// Wave 7.0.7 — set/clear the "Password reset required" flag on a reviewer.
// Used by (a) the bcrypt backfill script to lock plain-text rows into a
// forced-reset state, and (b) the /api/auth/reset-password endpoint to clear
// the flag after the reviewer chooses a new password.
...
// Wave 7.0.7 — atomic write of both the new password hash AND the reset flag.
// Used by the reset-password endpoint so we never leave a row with a new
// password but still flagged "reset required," or vice versa.
...
// Update the PDF file property on an intake study
