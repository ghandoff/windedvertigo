...
function PcsClaims() {
  ...
  const columns = [
    { key: 'claimNo', label: '#' },
    {
      ...
      label: 'Claim',
      render: (val, row) => (
        <Link href={`/research/pcs/claims/${row.id}`} className="max-w-[400px] inline-block truncate text-pacific-600 hover:underline" title={val}>
          {val}
        </Link>
      ),
    },
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-gray-900">{title}</h1>
        {(bucket || noEvidence || versionId) && (
          <Link href="/research/pcs/claims" className="text-sm text-pacific-600 hover:underline">
            Show all
          </Link>
        )}
      </div>
      ...
      <div className="flex items-center justify-between gap-3 rounded-lg border border-pacific-200 bg-pacific-50/60 px-4 py-2.5 text-sm">
        ...
        <Link
          href="/research/pcs/canonical-claims/backfill-review"
