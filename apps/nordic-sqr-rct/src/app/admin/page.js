    ...
    const fetchData = async () => {
      try {
        ...
        if (analyticsRes.ok) {
          ...
          setStats(prev => ({
            ...prev,
            totalStudies: analyticsData.summary?.totalArticles || 0,
          }));
        }
      } catch (error) {
    ...
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-8 sm:px-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
          ...
          <Link
            href="/admin/sync"
            className="btn-ghost flex items-center justify-center gap-2 py-3 rounded-lg border border-pacific text-pacific"
          >
            <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
