  ...
  const fetchData = async () => {
    try {
      ...
      if (studyId) {
        ...
        const allScoresRes = await fetch(`/api/scores?studyId=${studyId}`);
        if (allScoresRes.ok) {
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 flex-1 w-full">
        <div className="flex items-center justify-between mb-6">
          <Link href="/reviews" className="inline-flex items-center gap-2 text-pacific-600 hover:text-pacific font-medium text-sm">
          ...
          <a
            href={`/api/sqr/export/${scoreId}`}
            className="btn-primary inline-flex items-center gap-2 text-sm"
            title="Download a Cochrane RoB 2 formatted Word report for this assessment"
          >
            <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
