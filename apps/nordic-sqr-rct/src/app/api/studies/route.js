...
export async function GET(request) {
  try {
    // Wave 7.5 Batch C — listing all studies (and the catalog) requires
    // `sqr.scores:read-all` since it surfaces aggregate review state.
    const gate = await requireCapability(request, 'sqr.scores:read-all', { route: '/api/studies' });
...
export async function POST(request) {
  try {
    // Wave 7.5 Batch C — submitting a new study is a reviewer/researcher
    // action; gate on `sqr.scores:create-own` (held by reviewers + above).
    const gate = await requireCapability(request, 'sqr.scores:create-own', { route: '/api/studies' });
    if (gate.error) return gate.error;
    const { user } = gate;
