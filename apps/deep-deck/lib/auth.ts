...
/**
 * Build the Auth.js database adapter only when POSTGRES_URL is available.
 * Without a DB, auth is disabled and the app falls back to localStorage-only access.
 */
function buildAdapter(): NextAuthConfig["adapter"] {
  if (!hasDb) return undefined;

  // Lazy-require DB modules so they don't crash at import when POSTGRES_URL is missing
  const { sql } = require("@vercel/postgres");
  const { getUserByEmail, createUser, getUserById, updateUser } = require("@/lib/queries/users");

  return {
    async createUser(user: { email?: string | null; name?: string | null }) {
...
export const authConfig: NextAuthConfig = {
  ...
  callbacks: {
    async jwt({ token, user, account }) {
      ...
      if (account?.provider === "google" && token.userId && hasDb) {
        try {
          const { updateUser } = require("@/lib/queries/users");
