...
-- purchases (connected to Stripe Checkout)
CREATE TABLE purchases (
  ...
  currency        TEXT DEFAULT 'USD',
  payment_provider TEXT DEFAULT 'stripe',        -- 'stripe' | 'stub' (legacy/test)
...
Daily (every 24 hours at midnight UTC), plus a manual "sync now" button in the admin UI. Content changes are editorial, not real-time; daily is sufficient for MVP. Schedule can be tightened later if needed.
...
> **note**: this section captures the original build plan. For current completion status and detailed progress, see [`PROJECT-STATUS.md`](PROJECT-STATUS.md).
...
*completed — see PROJECT-STATUS.md for current status:*
- ~~Stripe integration for real purchases~~ (done — Stripe Checkout with webhook → purchase → entitlement flow)
- ~~SSO providers via Auth.js~~ (done — Google Workspace OAuth added session 4)
- ~~Team management UI within orgs~~ (done — invite flows, role-based access added session 5)
- ~~Run analytics and dashboards~~ (done — session tracking, completion rates added session 5)
- ~~Notion webhook listener~~ (done — incremental sync replaces polling, session 5)
...
## appendix 0: local development paths

| machine | path |
|---------|------|
| Garrett's desktop | `/Users/garrettjaeger/Projects/creaseworks` |
| Cowork VM mount | `/sessions/kind-happy-dijkstra/mnt/Projects--creaseworks` |

the `.git` directory is not visible through the cowork mount. git operations
(commit, push, pull, rebase) must be run by the user in their local terminal.
if the mount goes stale, use `request_cowork_directory` to re-mount the host
path `/Users/garrettjaeger/Projects/creaseworks`.
...
## appendix G: workflow preferences

- **commit messages**: always provide a descriptive commit comment for each push. Format: short summary line, then a blank line, then bullet points grouping changes by MVP or feature area.
