      ...
      {pattern.has_find_again && (
        <section className="rounded-xl border border-redwood/20 bg-redwood/5 p-6">
          <h2 className="text-sm font-semibold text-redwood mb-2">
            includes find again mode
          </h2>
          <p className="text-sm text-cadet/70 mb-4">
            this pattern includes a guided find-again prompt — a structured way to
            revisit and deepen the experience after your first run. find-again
            prompts are available when you unlock the full pattern through a pack
            purchase.
          </p>
          <Link
          ...
          >
            {pack ? `get ${pack.title}` : "view packs"}
          </Link>
        </section>
      )}
    </main>
