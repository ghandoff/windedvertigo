...
// RESERVOIR GAMES
// ============================================
async function fetchReservoirGames() {
  const propMap = config.properties.reservoirGames;
  const required = config.required.reservoirGames;

  const response = await withRetry(
    () => notion.databases.query({
      database_id: config.databases.reservoirGames,
      sorts: [{ property: propMap.order, direction: 'ascending' }],
    }),
    'fetchReservoirGames'
  );
  ...
  for (const page of response.results) {
    if (!validatePage(page, required, 'Reservoir Games')) {
      skipped++;
      continue;
    }
  ...
  console.log('  OK Reservoir Games: ' + games.length + ' games' + (skipped ? ' (' + skipped + ' skipped)' : ''));
  return games;
}
...
async function main() {
  ...
  try {
    const [quadrants, outcomes, portfolioAssets, vaultActivities, siteContent, reservoirGames] = await Promise.all([
      fetchQuadrants(),
      fetchOutcomes(),
