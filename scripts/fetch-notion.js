...
async function main() {
  ...
  try {
    const [quadrants, outcomes, portfolioAssets, vaultActivities, siteContent, harbourGames] = await Promise.all([
      fetchQuadrants(),
      fetchOutcomes(),
    ...
    if (harbourGames.length > 0) {
      const harbourDataDir = path.join(__dirname, '..', 'apps', 'harbour', 'data');
