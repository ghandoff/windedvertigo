...
async function fetchVertigoVault() {
  ...
  for (const page of response.results) {
    ...
    // standalone vertigo-vault app at /harbor/vertigo-vault.
...
// HARBOR GAMES
// ============================================
async function fetchHarborGames() {
  const propMap = config.properties.harborGames;
  const required = config.required.harborGames;

  const response = await withRetry(
    () => notion.databases.query({
      database_id: config.databases.harborGames,
      sorts: [{ property: propMap.order, direction: 'ascending' }],
    }),
    'fetchHarborGames'
  );
  ...
  for (const page of response.results) {
    if (!validatePage(page, required, 'Harbor Games')) {
      skipped++;
      continue;
    }
  ...
  console.log('  OK Harbor Games: ' + games.length + ' games' + (skipped ? ' (' + skipped + ' skipped)' : ''));
  return games;
}
...
// results by Page value (what, we, do, home, harbor, etc.).
...
async function main() {
  ...
  try {
    const [quadrants, outcomes, portfolioAssets, vaultActivities, siteContent, harborGames] = await Promise.all([
      fetchQuadrants(),
      fetchOutcomes(),
      fetchPortfolioAssets(),
      fetchVertigoVault(),
      fetchSiteContent(),
      fetchHarborGames().catch(err => {
        console.warn('  Warning: Harbor Games sync skipped (' + err.message + ')');
        return [];
      }),
    ]);
    ...
    // Write Harbor Games (skip if fetch failed gracefully)
    let harborGamesPath = null;
    if (harborGames.length > 0) {
      const harborDataDir = path.join(__dirname, '..', 'apps', 'harbor', 'data');
      if (!fs.existsSync(harborDataDir)) {
        fs.mkdirSync(harborDataDir, { recursive: true });
      }
      harborGamesPath = path.join(harborDataDir, 'games.json');
      fs.writeFileSync(harborGamesPath, JSON.stringify(harborGames, null, 2));
    }

    // Write Harbor Credibility data (extracted from Site Content CMS "harbor" page)
    const harborSections = siteContent.harbor || [];
    if (harborSections.length > 0) {
      const credentials = harborSections
        .filter(s => s.type === 'credential' && s.section === 'why')
        .sort((a, b) => a.order - b.order)
        .map(s => ({ icon: s.icon, label: s.name, detail: s.content }));
      const principles = harborSections
        .filter(s => s.type === 'principle' && s.section === 'principles')
        .sort((a, b) => a.order - b.order)
        .map(s => ({ heading: s.name, body: s.content }));
      const bioSection = harborSections.find(s => s.type === 'body' && s.section === 'bio');
      const heroSection = harborSections.find(s => s.section === 'hero' && s.type === 'hero');
      const whyHeader = harborSections.find(s => s.section === 'why-header');
      const ctaSection = harborSections.find(s => s.section === 'closing-cta' && s.type === 'cta');
      ...
      const credibilityPath = path.join(__dirname, '..', 'apps', 'harbor', 'data', 'credibility.json');
      fs.writeFileSync(credibilityPath, JSON.stringify(credibilityData, null, 2));
      console.log('  Harbor Credibility: ' + credentials.length + ' credentials, ' + principles.length + ' principles → ' + credibilityPath);
    }
    ...
    if (harborGamesPath) {
      console.log('  Harbor Games: ' + harborGames.length + ' games → ' + harborGamesPath);
    } else {
      console.log('  Harbor Games: skipped (database not accessible)');
    }
