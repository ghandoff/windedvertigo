#!/usr/bin/env bash
# Deploy the wv-ops Vercel project from the monorepo root.
#
# Vercel needs the full monorepo to resolve workspace packages
# (e.g. @windedvertigo/tokens). This script temporarily swaps
# .vercel/project.json to target wv-ops, deploys, then restores.
#
# Usage:
#   ./scripts/deploy-ops.sh            # production deploy
#   ./scripts/deploy-ops.sh --preview  # preview deploy
...
OPS_PROJECT_ID="prj_kmYvoUECyaAxND4GHj1WmiePGzU3"
OPS_ORG_ID="team_wrpRda7ZzXdu7nKcEVVXY3th"
...
echo "→ Swapping .vercel/project.json to target wv-ops"
...
printf '{"projectId":"%s","orgId":"%s","projectName":"wv-ops"}\n' \
  "$OPS_PROJECT_ID" "$OPS_ORG_ID" > "$PROJECT_JSON"
